(function (jq) {
    ...
    function initialize(pager) {
        ...
        $.ajax({
            ...
            success: function (response) {
                $.Hide('#shade,#loading');
                if (response.status) {
                    initGlobal(response.data.global_dict);  // 资产表中，外键choice字段，定义全局变量。(为显示文本和编辑使用)
    ...
    function initGlobal(globalDict) {
        $.each(globalDict, function (k, v) {
            NB_GLOBAL_DICT[k] = v;
            // window.NB_GLOBAL_DICT = {};  //也可以生成全局变量
            // NB_GLOBAL_DICT[k] = v
        })
    }
    ...
    function initTableBody(startNum, list, tbConfig) {
        ...
        $.each(list, function (k1, row) {
            // row 表示从数据库获取的每一行资产字典信息 {'id':'1','name': 'root' ...}
            // tbConfig 包含了所有表格的配置
        ...
        */
                    ...
                    $.each(config.text.kwargs, function (k, v) {
                        if (v.substring(0,2)==="@@") {
                            var global_name = v.substring(2, v.length); // 全局变量的名称，就是choice字段，数字和文本
                            kwargs[k] = getNameByGlobalList(global_name, row[config.q]);
                            //row[config.q] 获取到的，数据库中存储的数字类型的值。(根据值去对应的choice中拿到文本信息)
    ...
    /*
    *   [{'id': 1, 'name': '上架'}, {'id': 2, 'name': '在线'}, {'id': 3, 'name': '离线'}, {'id': 4, 'name': '下架'}]
        [{'id': 1, 'name': '服务器'}, {'id': 2, 'name': '交换机'}, {'id': 3, 'name': '防火墙'}]
        [{'id': 1, 'name': '兆维-1'}, {'id': 2, 'name': '石家庄-5'}, {'id': 3, 'name': '鹿泉-5'}]
        [{'id': 2, 'name': '二手车'}, {'id': 3, 'name': '咨询'}, {'id': 1, 'name': '车商城'}]
    * */
    ...
    function getNameByGlobalList(globalName, itemId) {
        ...
        $.each(NB_GLOBAL_DICT[globalName], function (k, v) {
            if (v.id == itemId) {
                result = v.name;
                return false;  // js 的return 只是，终止了 each 这个循环。不会终止整个函数。
            }
        });
        return result;
    }
    ...
    function DoTrIntoEdit($tr, specialInEditFunc) {
        $tr.find('td[edit-enable="true"]').each(function () {
            //$(this) 是，每一行中 edit-enable="true" 的td 标签。
    ...
    function ExecuteTdIntoEdit($td, specialInEditFunc) {
        //$td 是，每一行中 edit-enable="true" 的td 标签。
    ...
    jq.extend({
        ...
        'BindDoSingleCheck': function (tableBody, specialInEditFunc, specialOutEditFun) {
            $(tableBody).delegate(':checkbox', 'click', function () {
                var $tr = $(this).parent().parent();  // 获得每一行的 tr 标签。
                if ($(this).prop('checked')) {
                    if ($(tableBody).attr('edit-mode') == 'true') {
